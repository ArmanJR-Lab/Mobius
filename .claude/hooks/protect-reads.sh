...
case "$TOOL_NAME" in
  ...
  Read|Write|Edit)
    ...
    if ! check_path "$FILE_PATH"; then
      deny "Access denied: You can't access $FILE_PATH. It is outside your working directory."
    fi
    ;;
  ...
  Glob|Grep)
    ...
    if ! check_path "$SEARCH_PATH"; then
      deny "Access denied: You can't access $SEARCH_PATH. It is outside your working directory."
    fi
    ;;
esac
